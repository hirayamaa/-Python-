...
for quiz_num in range(2):
    # 問題集と解答集のファイルを作成する
    quiz_file = open(f'randomQuiz/capitalsQuiz{quiz_num + 1}.txt', 'w')
    answer_key_file = open(f'randomQuiz/capitalsQuiz_answers{quiz_num + 1}.txt', 'w')
    # 問題集のヘッダーをファイルに記載する
    quiz_file.write('名前:\n\n日付:\n\n学期:\n\n')
    quiz_file.write((' ' * 20)
    ...
    for question_num in range(len(prefectures)):
        ...
        answer_key_file.write("ABCD"[answer_option.index(correct_answer)])
